...
SoundBuffer::SoundBuffer(const char* filename)
{
	...
	m_SoundID = addSoundEffect(filename);
}
...
std::shared_ptr<SoundBuffer> SoundBuffer::Create(const char* filename)
{
	return std::shared_ptr<SoundBuffer>(new SoundBuffer(filename));
}
